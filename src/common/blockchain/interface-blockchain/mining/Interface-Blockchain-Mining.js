    ...
    async mineBlock( block,  difficulty, start, end, height, showLogsOnlyOnce){
        ...
        try{
            ...
            if (answer && answer.result && this.blockchain.blocks.length === block.height ){
                ...
                console.warn( "----------------------------------------------------------------------------");
                console.warn( "IDLL Block was mined ", block.height ," nonce (", answer.nonce+")", "timestamp", block.timeStamp, answer.hash.toString("hex"), " reward", (block.reward / WebDollarCoins.WEBD), "WEBD", block.data.minerAddress.toString("hex"));
